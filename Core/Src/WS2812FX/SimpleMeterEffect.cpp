...
void SimpleMeterEffect::updateEffect()
{
	...
	uint8_t flip0 = *(static_cast<uint8_t *>(this->getParameter(2)->getValue()));
	...
	uint8_t flip1 = *(static_cast<uint8_t *>(this->getParameter(5)->getValue()));
	uint8_t mirror0 = *(static_cast<uint8_t *>(this->getParameter(6)->getValue()));
	uint8_t mirror1 = *(static_cast<uint8_t *>(this->getParameter(7)->getValue()));
	...
	{
		WS2812_SimpleMeterEffect(rgb0, meterFill0, (bool)flip0);
	}
	...
	{
		WS2812_SimpleMeterEffect(rgb1, meterFill1, (bool)flip1);
	}
